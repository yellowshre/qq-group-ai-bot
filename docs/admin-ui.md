...
控制台会对会写数据库、发布正式知识、生成 embedding 或刷新缓存的操作做二次确认。当前覆盖：

- 群配置保存。
- 表情包场景保存、素材保存、缓存预热。
- 聊天 JSON 导入、候选生成、候选批量审批。
- 候选发布到正式知识库 / 成员画像。
- 正式知识 embedding 生成。

只读查询、路径巡检、召回预览、Dify inputs 预览和日志查询不会弹确认。确认弹窗只用于防误触，不会改变后端权限校验；实际写入仍由 dev/local 管理接口和 Admin API token 配置控制。
...
页面会在保存前显示变更预览，列出本次保存会修改的字段、旧值和新值。没有变更时不会提交更新请求，避免误保存。点击“保存配置”后还会出现二次确认，确认后才会调用更新接口。
